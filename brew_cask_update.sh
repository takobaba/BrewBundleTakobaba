...
{
  echo
  echo "${BLACK}Check/create backup directory ${RED}$BACKUPDIR${NC}"
} 2>/dev/null
[[ ! -e "$BACKUPDIR" ]] && mkdir "$BACKUPDIR"

{
  echo
  echo "${BLACK}Creating backup directory ${RED}$BACKUPDIR/$DATE${NC}"
} 2>/dev/null
mkdir -p "$BACKUPDIR/$DATE"

{
  echo
  echo "${BLACK}Backing up current cask files${NC}"
} 2>/dev/null
...
{
  echo
  echo "${BLACK}Rename ${BLUE}$INSTALLED${BLACK} with ${BLUE}$CURRENT${NC}"
} 2>/dev/null
...
{
  echo
  echo "${BLACK}Replace cask script with latest version${NC}"
} 2>/dev/null
